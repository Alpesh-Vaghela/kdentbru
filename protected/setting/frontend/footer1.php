...
<?php if ($query1ans == "calendar") { ?>
    ...
    <script>
        function reload_calender() {
            ...
            var new_url = "index.php?user=calendar&c_date=" + new_d + "&cal_view=" + cal_view.name;
        ...
        function load_calender(c_date) {
            ...
            <?php
            if ($_SESSION['user_type'] == "staff") {
                $alla = $db->get_all('appointments', array('company_id' => CURRENT_LOGIN_COMPANY_ID, 'staff_id' => $_SESSION['user_id'], 'private' => 'no'));
            } elseif ($_SESSION['user_type'] == "receptionist") {
                if (isset($_REQUEST['loadfor'])) {
                    $alla = $db->get_all('appointments', array('company_id' => CURRENT_LOGIN_COMPANY_ID, 'staff_id' => $_REQUEST['loadfor']));
                } else {
                    $alla = $db->get_all('appointments', array('company_id' => CURRENT_LOGIN_COMPANY_ID));
                }
            } else {
                if (isset($_REQUEST['loadfor'])) {
                    $alla = $db->get_all('appointments', array('company_id' => CURRENT_LOGIN_COMPANY_ID, 'staff_id' => $_REQUEST['loadfor']));
                } else {
                    $alla = $db->run("SELECT * FROM `appointments` WHERE `company_id`= " . CURRENT_LOGIN_COMPANY_ID . " AND `appointment_date` >= CURDATE() AND `status` != 'deleted' ")->fetchAll();
                }
            }
            $doctors = $db->get_all('users', array('user_type' => 'staff'));
            $doc_arr = $doc_arrw = $doc_arrn = array();
            $i = 0;
            foreach ($doctors as $d) {
                $fullname = $d['firstname'] . " " . $d['lastname'];
                $doc_arr[$i] = $fullname;
                $doc_arrw[$i] = $d['firstname'][0];
                $doc_arrn[$fullname] = $i;
                $i++;
            }
            ?>
            $('#demo-calendar12').fullCalendar({
                ...
                views: {
                    multiColAgendaDay: {
                        type: 'multiColAgenda',
                        duration: {days: 1},
                        numColumns: <?php echo count($doc_arr);?>,
                        columnHeaders: [<?php echo '"' . implode('","', $doc_arr) . '"' ?>]
                    },
                    multiColAgendaWeek: {
                        type: 'multiColAgenda',
                        duration: {weeks: 1},
                        numColumns: <?php echo count($doc_arrw);?>,
                        columnHeaders: [<?php echo '"' . implode('","', $doc_arrw) . '"' ?>]
                    }
                },
                allDaySlot: false,
                ...
                events:
                    [
                        ...
                        $service_provider_fullname = $service_provider_firstname . " " . $service_provider_lastname;
                        ?>
                        {
                            title: "<?php echo ucfirst($service_provider_firstname);?> <?php echo $service_provider_lastname;?> Customer: <?php echo $customer_fullname . $pathology; ?> Notes: <?php echo $appointment_note?>",
                            ...
                            appointment_date_service_time: '<?php
                                ...
                                setlocale(LC_TIME, 0);
                                echo $AppDateTime;?> <?php echo date('h:i a', strtotime($appointment_time));?> - <?php echo date('h:i a', strtotime($appointment_end_time));?>',
                            provider: "Dottore: <?php echo $service_provider_fullname;?>",
                            service: "Prestazione: <?php echo $service_name?>",
                            cost: "Costo: <?php echo CURRENCY . "" . $service_cost?>",
                            customer: "<strong>Paziente: <?php echo $customer_fullname?></strong>",
                            mobile: "Mobile No: <?php echo $customer_phone; ?>",
                            notes: "Note: <?php echo $appointment_note?>",
                            status: "Stato: <?php echo $appointment_status?>",
                            allDay:false,
                            column: <?php echo !empty($doc_arrn[$service_provider_fullname]) ? $doc_arrn[$service_provider_fullname] : "0"?>
                        },
                        <?php $i++;
                        } ?>
                        <?php }
