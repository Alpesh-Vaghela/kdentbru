    ...
    <div class="row">
        ...
        <div class="col-lg-4 col-md-4 col-sm-6 col-xm-12">
            ...
            <div class="media-body">
                ...
                <p class="h5 mar-no text-center"><?php echo  date(COMMON_DATE_FORMAT,strtotime($week_start_date));?> / <?php echo date(COMMON_DATE_FORMAT,strtotime($week_end_date));?></p>
            </div>
               ...
               <div class="form-group">
                   <div class="col-md-6">
                     ...
                     <select class="form-control12" name="filter_by_dashboard" id="filter_by_dashboard" >
                         <option <?php if ($filter_by_dashboard=="thisweek"){echo "selected='selected'";}?> value="thisweek">Questa settimana</option>
                         <option <?php if ($filter_by_dashboard=="today"){echo "selected='selected'";}?> value="today">Oggi</option>
                         <option <?php if ($filter_by_dashboard=="nextweek"){echo "selected='selected'";}?> value="nextweek">Prossima settimana</option>
                         <option <?php if ($filter_by_dashboard=="thismonth"){echo "selected='selected'";}?> value="thismonth">Questo mese</option>
                         <option <?php if ($filter_by_dashboard=="nextmonth"){echo "selected='selected'";}?> value="nextmonth">Prossimo mese</option>
                     </select>
                 </div>
             </div>
        ...
        <?php if (!empty($this_week_appointments))
        {
            foreach ($this_week_appointments as $appoint)
            {
                ...
                <tr>
                    ...
                    <td width="45%">
                        <?php echo date(COMMON_DATE_FORMAT,strtotime($appoint['appointment_date']));?>(<?php echo date('h:i  A', strtotime($appoint['appointment_time']))."-".date('h:i  A', strtotime($appoint['appointment_end_time']));?>)
                        <br><?php echo ucwords($service_provider_firstname." ".$service_provider_lastname);?> .Customer Name: <?php echo $customer_name.$pathology; ?> . <?php echo ucfirst($service_name);?> . <?php echo $appoint['service_time']+$appoint['service_buffer_time']?> min . <?php echo CURRENCY."".$appoint['service_cost']?>
                    </td>
                    <td width="30%">
                        <?php if($appoint['status'] == 'deleted'): ?>
                            <p style="font-size: 12px;"><b>Motivo cancellazione: </b><?php echo $appoint['cancel_reason']; ?></p>
                        ...
                        <?php if($appoint['status'] != VISIT_IN_PRECESS && $appoint['status'] != VISIT_DONE && $appoint['status']!='deleted') { ?>
                         <a class="label label-info" href="<?php echo $link->link("home",user,'&action_update='.$appointment_id);?>"><i class="fa fa-edit">Visita Eseguita</i></a>
                     <?php }
                     if($appoint['status']=="pending"){
                      if (strtotime($appoint['appointment_date']." ".$appoint['appointment_time']) > strtotime(date('Y-m-d H:i')))
                       ...
                       <?php }else{?>
                        <span class="label label-danger">in Ritardo/non Presentato</span><br>
                        <a data-toggle="modal" data-target="#myModal_edit_appointment" data="<?php echo $appoint['id'];?>" class="label label-primary edit_modal_edit_customer" style="margin-top: 10px;margin-bottom: 10px"><i class="fa fa-refresh"> Reschedule Appointment</i></a>
                    <?php }
                     }
                   ...
                   <span class="label class-<?php echo $appoint['id']; ?> label-<?php if ($appoint['status']=="pending"){echo"warning";}else{echo "success";} ?>"><i class="fa fa-tag"></i>
                    <?php
...
if ($appoint['payment_status']=="unpaid"){?>
   ...
   <a href="#" data-toggle="modal" data-target="#myModal_payment" id="load_payment_details" class="load_payment_details"
   data_id="<?php echo $appoint['id'];?>"
   data_booking_id="<?php echo $appoint['booking_id'];?>"
   data_booking_date="<?php echo date(COMMON_DATE_FORMAT,strtotime($appoint['appointment_date']));?>" 
   data_customer="<?php echo ucwords($customer_name);?>"
...
<div class="col-md-3">
    <div class="panel">
        <div class="panel-heading">
            <h4 class="panel-title">ATTIVITA' <small>(<a  class="text-danger"  href="<?php echo $link->link('activity',frontend);?>">Dettagli</a>)</small></h4>
        </div>
        <div class="panel-body np">
            <!--Chat widget-->
            <!--===================================================-->
                      ...
                      <ul class="list-group bg-trans"> 
                         ...
                         if (!empty($this_week_activity)){
                            foreach ($this_week_activity as $alla)
                            {
                               ...
                               <div class="inline-block">
                                ...
                                <small class="text-muted">
                                    <?php 
                                                        ...
                                                        By <b><?php  echo $staff_first_name=$db->get_var('users',array('user_id'=>$alla['user_id']),'firstname');
                                                        echo " ";
                             ...
                             <div class="nano-pane"><div class="nano-slider" style="height: 92px; transform: translate(0px, 0px);"></div></div></div>
                             <!--Widget footer-->
                                ...
                                <tbody>
                                    ...
                                    if (is_array($memo_notification)){
                                        foreach ($memo_notification as $allmn){?>
                                            <tr>
                                                <td><strong class="text-primary"><?php echo date(COMMON_DATE_FORMAT,strtotime($allmn['date']));?> </strong><a data-toggle="modal" data-target="#myModal_edit_memo" data="<?php echo $allmn['id'];?>" class="edit_memo" ><i class="fa fa-edit fa-2x text-info"></i></a>
                                                    <?php if ($allmn['status'] == "0") { ?>
                                                        <a class="label label-info" href="<?php echo $link->link("home",user,'&action_complete='.$allmn['id']);?>">Archivia</a>
                                                    <?php } ?>
                                                    <br>
