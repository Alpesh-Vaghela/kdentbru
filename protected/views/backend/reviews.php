...
if ($current_tab!="preferences" && $current_tab!="pending" && $current_tab!="published" && $current_tab!="hidden")
   ...
   {
       $reviews=$db->get_all('review',array('company_id'=>$company_id,'status'=>'pending'));
       
   }
   elseif ($current_tab=="published")
   {
       $reviews=$db->get_all('review',array('company_id'=>$company_id,'status'=>'publish'));
      
   }
   elseif ($current_tab=="hidden")
   {
       $reviews=$db->get_all('review',array('company_id'=>$company_id,'status'=>'hide'));
       
   }
   $hide_count=$db->get_count('review',array('company_id'=>$company_id,'status'=>'hide'));
   $publish_count=$db->get_count('review',array('company_id'=>$company_id,'status'=>'publish'));
   $pending_count=$db->get_count('review',array('company_id'=>$company_id,'status'=>'pending'));
...
if (isset($_POST['update_preference_update_form_submit']))
{
    ...
    if ($update)
    {
        ...
        echo "<script>
         setTimeout(function(){
         window.location = '".$link->link("reviews",admin,'&action_id='.$company_id)."'
         },3000);</script>";
    }
...
if ($update){
    $session->redirect('reviews&action_id='.$company_id,admin);
}
                                          ...
                                          <div class="form-group">
            					           <div class="col-md-8">
                                                 <div class="checkbox">
                                                        ...
                                                        <h4>Enable Review</h4><input <?php if ($preferences_settings_details['enable_review']=="yes"){echo 'checked';}?>  type="checkbox" name="enable_review"  value="yes"></label><br>
                                                   ...
                                                   <td><?php echo date('D M d,Y',strtotime($review_detail['created_date']));?></td>
                                                    <td><?php echo ucwords($review_detail['review_provider_name']);?><br> <?php echo ucfirst($review_detail['review_detail']);?> </td>
                                                     ...
                                                     <td>
                                                         ...
                                                         <a href="<?php echo $link->link("reviews",admin,'&action_id='.$company_id.'&action_publish='.$review_detail['id']);?>"><i class="fa fa-check  text-success"></i></a>
                                                         <?php }
                                                         if ($review_detail['status']=="pending" || $review_detail['status']=="publish" ){?>
                                                         <a href="<?php echo $link->link("reviews",admin,'&action_id='.$company_id.'&action_hide='.$review_detail['id']);?>"><i class="fa fa-eye-slash  text-info"></i></a>
                                                         <?php }?>
                                                         <a href="<?php echo $link->link("reviews",admin,'&action_id='.$company_id.'&action_delete='.$review_detail['id']);?>"><i class="fa fa-trash  text-danger"></i></a>
                                                         </td> 
