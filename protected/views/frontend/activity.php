...
if(isset($_REQUEST['action_delete']))
{
  ...
  if(isset($_POST['yes']))
  {
    ...
    if($delete)
    {
      ...
      echo "<script>
      setTimeout(function(){
       window.location = '".$link->link("activity",user)."'
     },3000);</script>";
   }
 }
 elseif(isset($_POST['no']))
 {
  $session->redirect("activity",user);
}
        ...
        <tbody>
          ...
          if (is_array($all_activity_log)){
            ...
            foreach ($all_activity_log as $alla)
            {
             ...
             ?></td>
             <td>
              <a class="btn btn-danger " href="<?php echo $link->link("activity",user,'&action_delete='.$alla['id']);?>"><i class="fa fa-trash"></i> Delete</a>
            </td>
