        ...
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group btn-group-sm">
                    ...
                    $SQL = "SELECT * FROM `users` WHERE `user_type`= 'staff' " . $mul_doctor_condition;
                    $doctors = $db->run($SQL)->fetchAll();
                    ?>
                    <select data-placeholder="Scegliere Dottore" class="form-control" multiple="multiple"
                            id="mul_doctor_id" onchange="reload_calender();">
                        <?php foreach ($doctors as $doctor) { ?>
                            <option
                                <?php echo in_array($doctor['user_id'], $mul_doctor_id_array) ? 'selected="selected"' : '' ?>
                                    value="<?php echo $doctor['user_id'] ?>">
                                <?php echo $doctor['firstname'] . " " . $doctors['lastname'] ?>
                            </option>
                        <?php } ?>
                    </select>
           ...
           <?php } ?>
                    ...
                    <ul class="dropdown-menu">
                        ...
                        $sf = $db->run("SELECT* FROM `users` WHERE `visibility_status`='active' AND `company_id`='$com_id' AND `user_type`='staff'" . $mul_doctor_condition)->fetchAll();
                        if (is_array($sf)) {
                            foreach ($sf as $alls) {
                                ?>
                                <li>
                                    <a href="<?php echo $link->link('calendar', frontend, '&loadfor=' . $alls[user_id]); ?>">
                                        <span><i class="fa fa-user"></i> <?php echo $alls['firstname'] . " " . $alls['lastname'] . ""; ?></span></a>
                                </li>
                            <?php }
                        } ?>
                        <?php if ($_SESSION['user_type'] == "admin") { ?>
                            <li class="text-info"><a href="" data-toggle="modal" data-target="#myModal"> <i
                                            class="fa fa-plus-circle"></i> Aggiungi DOTTORE </a></li>
                        <?php } ?>
                    </ul>
                </div>
                <?php if ($_SESSION['user_type'] == "admin") { ?>
                    <ul class="nav navbar-top-links pull-right">
                        ...
                        <li id="dropdown-user" class="dropdown">
                            <a href="#" data-toggle="dropdown" class="dropdown-toggle text-right"><i
                                        class="fa fa-plus-circle fa-2x text-danger"></i></a>
                            <div class="dropdown-menu dropdown-menu-right with-arrow">
                                <ul class="head-list">
                                    <li><a href="" data-toggle="modal" data-target="#myModal"> <i
                                                    class="fa fa-user"></i> Aggiungi nuovo DOTTORE </a></li>
                                    <li><a href="" data-toggle="modal" data-target="#myModal_add_sevice"> <i
                                                    class="fa fa-truck fa-fw"></i> Aggiungi nuova PRESTAZIONE </a></li>
                                    <li><a href="" data-toggle="modal" data-target="#myModal_add_customer"> <i
                                                    class="fa fa-user-secret fa-fw"></i> Aggiungi nuovo PAZIENTE</a>
                                    </li>
                                </ul>
                            </div>
                        </li>
                        <?php if (SHOW_CALENDAR_STATS == "yes") { ?>
                            <li id="dropdown-user" class="dropdown">
                                <a href="#" data-toggle="dropdown" class="dropdown-toggle text-right"><i
                                            class="fa fa-signal fa-2x text-danger"
                                            id="get_current_month_from_full_calendar"></i></a>
                                <div class="dropdown-menu dropdown-menu-right with-arrow">
                                    <ul class="head-list">
    ...
    <div class="row">
        <div class="col-md-12 col-lg-12">
            <div class="text-right form-group">
                <button class="fc-button fc-state-default fc-corner-left fc-corner-right" type="button"
                        onClick="reload_calender()">Refresh
                </button>
            </div>
            <div class="panel">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-danger change-text-app" id="update_arrival_appointment_calendar"><i
                            class="fa fa-edit"></i> Paziente ARRIVATO
                </button>
                <button class="btn btn-info" id="load_edit_appoint_form" data-toggle="modal"
                        data-target="#myModal_edit_appointment"><i class="fa fa-edit"></i> Modifica
                </button>
                <button data-toggle="modal" data-target="#myModal_cancel_appointment"
                        class="btn btn-danger cancel_modal_cancel_customer cancel-calender"><i class="fa fa-trash"></i>
                    Cancella
                </button>
        ...
        <div class="modal-content">
            ...
            <form id="add_appointment_form" method="post" class="form-horizontal" action=""
                    ...
                    <div class="form-group">
                            ...
                            <select class="form-control" name="customer_id" id="loadnewcus">
                                ...
                                if (is_array($allcustomers)) {
                                    foreach ($allcustomers as $cus) {
                                        ?>
                                        <option <?php if ($_REQUEST['bookig_for_customer'] == $cus['id']) {
                                            echo "selected='selected'";
                                        } ?> value="<?php echo $cus['id'] ?>"><?php echo $cus['first_name'] . " " . $cus['last_name']; ?>
                                            )
                                        </option>
                                    <?php }
                                } ?>
                        ...
                        <div class="col-md-1">
                            <button type="button" class="btn btn-info pull-right" data-toggle="modal"
                                    data-target="#myModal_add_customer"><i class="fa fa-plus"></i></button>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <select class="form-control" name="service_provider"
                                    id="load_services_by_provider" <?php if ($loadfor != "") {
                                echo "readonly";
                            } ?>>
                                ...
                                <?php } else { ?>
                                    ...
                                    <option value="">---Seleziona---</option>
                                    <?php
                                    $provider = $db->run("SELECT * FROM `users` WHERE `user_type`= 'staff' AND `visibility_status`='active' AND `company_id`='" . CURRENT_LOGIN_COMPANY_ID . "' " . $mul_doctor_condition)->fetchAll();
                                    if (is_array($provider)) {
                                        foreach ($provider as $pro) {
                                            ?>
                                            <option <?php if ($loadfor == $pro['user_id']) {
                                                echo "selected='selected'";
                ...
                <div class="modal-footer">
                    ...
                    <button type="reset" class="btn btn-default" data-dismiss="modal"><i class="fa fa-times"></i> Close
                    </button>
                </div>
            </form>
        </div>
