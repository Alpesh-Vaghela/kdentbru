...
{
    $room_name="All Rooms";
    $c_id=CURRENT_LOGIN_COMPANY_ID;
    $bedquery="SELECT * FROM `appointments` WHERE `company_id`='$c_id' AND `assigned_room`!='0'";
...
if (isset($_POST['add_room_submit']))
{
    ...
    if ($insert)
    {
        echo "<script>
        setTimeout(function(){
           window.location = '".$link->link("rooms_seats",frontend)."'
       },1000);</script>";
   }
}
}
if(isset($_REQUEST['action_delete']))
{
    ...
    { 
        ...
        if($delete)
        {
            ...
            echo "<script>
            setTimeout(function(){
               window.location = '".$link->link("rooms_seats",user)."'
           },3000);</script>";
       }
   }
   elseif(isset($_POST['no']))
   {
    $session->redirect('rooms_seats',user);
}
            ...
            <div class="panel-body"> 
              ...
              <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <?php 
                    $all_category=$db->get_all('rooms',array('visibility_status'=>'active','company_id'=>CURRENT_LOGIN_COMPANY_ID,));
                    if (is_array($all_category)){
                       foreach ($all_category as $allc)
                       {
                           ...
                           ?>
                           <tr>
                            <td><h4><a  href="<?php echo $link->link('rooms_seats',frontend,'&action_view_status='.$allc['id']);?>"><?php echo ucwords($allc['name']);?>&nbsp;&nbsp;<span class="badge badge-info"><?php echo $count_service;?></span></a></h4></td>
                            <td></td>
                            <td>
                                <a class="text-info" href="<?php echo $link->link('rooms_seats',frontend,'&action_edit='.$allc['id']);?>"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                                <a class="text-danger" href="<?php echo $link->link('rooms_seats',frontend,'&action_delete='.$allc['id']);?>"><i class="fa fa-trash"></i></a></td>
                            </tr>
                        <?php }}?>
        ...
        foreach ($allappointments as $value){
            ...
            <tr>
             ...
             <td>
              ...
              <?php echo CURRENCY."".$value['service_cost']?></td>
